...
/** Écran de fin : carte fixe, logo et ligne de fin de la marque. */
export const Ending = ({ brand, product, start }: Props) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const L = useLayout();

  const t = frame - start;
  const veil = interpolate(t, [0, fps * 0.6], [0, 0.55], {
